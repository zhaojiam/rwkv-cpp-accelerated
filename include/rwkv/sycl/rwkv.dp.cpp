...
void logData(DTYPE* input, int N = 1, std::string logname = "log"){
    ...
    dpct::get_in_order_queue().memcpy(outbuf2, input, N * sizeof(DTYPE)).wait();
...
void cuda_layernorm(unsigned long long n_emb, const double *__restrict__ const x, const double *__restrict__ const weight, unsigned long long offset, float *const inmean, float *const instd, double *__restrict__ const out, unsigned long long tokenlength,
                    const sycl::nd_item<3> &item_ct1)
{
    for(unsigned long long token = 0; token < tokenlength; token++){
        double xmean = double(inmean[token]) / n_emb;
        double x2 = sycl::sqrt(instd[token] / (n_emb - 1));

        unsigned long long ii =
            item_ct1.get_group(2) * (item_ct1.get_local_range(2) * EMBBLOCK);
        for (unsigned long long c = 0; c < EMBBLOCK; c++)
        {
            unsigned long long i = ii + item_ct1.get_local_id(2) * EMBBLOCK + c;
    ...
    const sycl::nd_item<3> &item_ct1)
{
    ...
    for (unsigned long long token = 0; token < tokenlength; token++)
    {
        ...
        const unsigned long long j0 = sycl::min(
            N,
            (unsigned long long)(item_ct1.get_group(2) *
                                 ((N + MM8_ONE_JSPLIT - 1) / MM8_ONE_JSPLIT)));
        const unsigned long long j1 = sycl::min(
            N,
            (unsigned long long)((item_ct1.get_group(2) + 1) *
                                 ((N + MM8_ONE_JSPLIT - 1) / MM8_ONE_JSPLIT)));
        ...
        if (k < N)
        {
            ...
            dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
                reinterpret_cast<float *>(&y[k + token * N]),
                *reinterpret_cast<float *>(&y_local));
            dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
                reinterpret_cast<float *>(&y1[k + token * N]),
                *reinterpret_cast<float *>(&y1_local));
            dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
                reinterpret_cast<float *>(&y2[k + token * N]),
                *reinterpret_cast<float *>(&y2_local));
        }
    }
}
...
{
sycl::range<3> blockSize(1, MM8_ONE_TILE, 1);
sycl::range<3> gridSize(1, (N + blockSize[1] - 1) / blockSize[1],
                        MM8_ONE_JSPLIT);
    /*
    ...
    */
    dpct::get_in_order_queue().parallel_for(
        ...
        [=](sycl::nd_item<3> item_ct1) {
            kernel_mm8_threec(N, xy, w, w1, w2, r, r1, r2, o1, o2, o3, y, y1,
                              y2, offset, tokenlength, item_ct1);
        });
}
    ...
    const sycl::nd_item<3> &item_ct1)
{
    ...
    for (unsigned long long c = 0; c < EMBBLOCK; c++)
    {
        ...
        if (i < emb)
        {
            out[i] = float(1.0 / (1.0 + sycl::exp(-double(a[i]))));
            d[i * 4] = 0.0;
            d[i * 4 + 1] = 0.0;
            d[i * 4 + 2] = 0.0;
            d[i * 4 + 3] = 0.0;
...
void kernel_wkvc_forward(const unsigned long long C,
                                    ...
                                    unsigned long long offset, unsigned long long layers, unsigned long long tokenlength,
                                    const sycl::nd_item<3> &item_ct1, MODE mode = PARRALEL)
{

    for( unsigned long long token = 0; token < tokenlength; token ++){

        long long i = item_ct1.get_group(2) * (item_ct1.get_local_range(2) * EMBBLOCK);
        for (unsigned long long c = 0; c < EMBBLOCK; c++)
        {
            long long ii = i + item_ct1.get_local_id(2) * EMBBLOCK + c;
            ...
            if (ii < C)
            {
                ...
                const double wr1 =
                    aa + sycl::exp(u[ii + C * offset] + w[ii + C * offset] +
                                   k[ii + token * C]) *
                             vv;
                const double wr2 =
                    bb + sycl::exp(u[ii + C * offset] + w[ii + C * offset] +
                                   k[ii + token * C]);
                y[ii + token * C] = (wr1) / wr2;
                y[ii + token * C] =
                    (1.0 / (1.0 + sycl::exp(-r[ii + token * C]))) *
                    y[ii + token * C];
                aa = (aa + sycl::exp(double(k[ii + token * C])) * vv) *
                     sycl::exp((double)(w[ii + C * offset]));
                bb = (bb + sycl::exp(double(k[ii + token * C]))) *
                     sycl::exp((double)(w[ii + C * offset]));
                _aa[stateoffset] = aa;
                _bb[stateoffset] = bb;
                _pp[stateoffset] = pp;
...
void cuda_wkvc_forward(unsigned long long B, double *w, double *u, float *k, float *v, float *r, double *y, double *aa, double *bb, double *pp, unsigned long long offset, unsigned long long layers, unsigned long long tokenlength, MODE mode = PARRALEL)
{
    ...
    dpct::get_in_order_queue().parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, (B + EMBSPLIT - 1) / EMBSPLIT) *
                              sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
                          sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
        [=](sycl::nd_item<3> item_ct1) {
            kernel_wkvc_forward(B, w, u, k, v, r, y, aa, bb, pp, offset, layers,
                                tokenlength, item_ct1, mode);
        });
}

template <typename DTYPE>
void kernelc_mm8_one(
    ...
    unsigned long long tokenlength, const sycl::nd_item<3> &item_ct1)
{
    ...
    for (unsigned long long token = 0; token < tokenlength; token++)
    {
        const unsigned long long k =
            ...
            item_ct1.get_local_id(1);
        /*
DPCT1064:2: Migrated min call is used in a macro/template definition and may not
...
*/
        const unsigned long long j0 = sycl::min(
            ...
            (unsigned long long)(item_ct1.get_group(2) *
                                 ((N + MM8_ONE_JSPLIT - 1) / MM8_ONE_JSPLIT)));
        /*
DPCT1064:3: Migrated min call is used in a macro/template definition and may not
be valid for all macro/template uses. Adjust the code.
*/
        const unsigned long long j1 = sycl::min(
            N,
        ...
        if (k < M)
        {
            ...
            dpct::atomic_fetch_add<sycl::access::address_space::generic_space>(
                reinterpret_cast<float *>(&y[k + M * token]),
                *reinterpret_cast<float *>(&y_local));
        }
    }
}
...
{
    sycl::range<3> blockSize(1, MM8_ONE_TILE, 1);
    sycl::range<3> gridSize(1, (M + blockSize[1] - 1) / blockSize[1],
                            MM8_ONE_JSPLIT);
    /*
DPCT1049:1: The work-group size passed to the SYCL kernel may exceed the limit.
...
*/
    dpct::get_in_order_queue().parallel_for(
        sycl::nd_range<3>(gridSize * blockSize, blockSize),
        [=](sycl::nd_item<3> item_ct1) {
            kernelc_mm8_one(N, M, x, w, w_stride, y, r, o, offset, tokenlength,
                            item_ct1);
        });
}
...
void addall(const unsigned long long emb, float *acc, double *a, unsigned long long tokenlength,
            const sycl::nd_item<3> &item_ct1)
{
    unsigned long long ii =
        item_ct1.get_group(2) * (item_ct1.get_local_range(2) * EMBBLOCK);
...
void variance(const unsigned long long emb, float *acc, double *a, float *mean, unsigned long long tokenlength,
              const sycl::nd_item<3> &item_ct1)
{
    unsigned long long ii =
        item_ct1.get_group(2) * (item_ct1.get_local_range(2) * EMBBLOCK);
...
sycl::queue &q_ct1 = dev_ct1.in_order_queue();
    float *acc = &buffer[0];
    q_ct1.memset(acc, 0, sizeof(float) * tokenlength).wait();
    {
        dpct::has_capability_or_fail(q_ct1.get_device(), {sycl::aspect::fp64});

        q_ct1.parallel_for(
            sycl::nd_range<3>(
                sycl::range<3>(1, 1, (emb + EMBSPLIT - 1) / EMBSPLIT) *
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                addall(emb, acc, a, tokenlength, item_ct1);
            });
    }
    float *mean = &buffer[tokenlength];
    q_ct1.memcpy(mean, acc, sizeof(float) * tokenlength);
    q_ct1.memset(acc, 0, sizeof(float) * tokenlength).wait();
    {
        dpct::has_capability_or_fail(q_ct1.get_device(), {sycl::aspect::fp64});
        ...
        q_ct1.parallel_for(
            sycl::nd_range<3>(
                ...
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                variance(emb, acc, a, mean, tokenlength, item_ct1);
            });
    }
    float *var = &buffer[2*tokenlength];
    q_ct1.memcpy(var, acc, sizeof(float) * tokenlength);
...
sycl::queue &q_ct1 = dev_ct1.in_order_queue();
    ...
    q_ct1.memcpy(logitsout, logitsin, 50277 * sizeof(float) * tokenlength).wait();
    q_ct1
        .memcpy(statexyout, statexyin,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(stateaaout, stateaain,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(statebbout, statebbin,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(stateppout, stateppin,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(stateddout, stateddin,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
};
...
sycl::queue &q_ct1 = dev_ct1.in_order_queue();
    ...
    q_ct1
        .memcpy(stateaa, instateaa,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(statebb, instatebb,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(statecc, instatecc,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(statedd, instatedd,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
    q_ct1
        .memcpy(stateee, instateee,
                n_embed * n_layers * sizeof(double) * tokenlength)
        .wait();
};
...
sycl::queue &q_ct1 = dev_ct1.in_order_queue();
        ...
        for(unsigned long long i = 0; i < tokenlength; i++){
            q_ct1
                .memcpy(&buffer3[i * n_emb], &embed[token[i] * n_emb],
                        n_emb * sizeof(float))
                .wait();
        }
    ...
    // copy buffer3 to buffer1
    {
        ...
        q_ct1.parallel_for(
            sycl::nd_range<3>(
                sycl::range<3>(
                    1, 1, (n_emb * tokenlength + EMBSPLIT - 1) / EMBSPLIT) *
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                setx(n_emb * tokenlength, buffer3, buffer1, item_ct1, 0);
            });
    }
...

    {
        ...
        q_ct1.parallel_for(
            sycl::nd_range<3>(
                sycl::range<3>(1, 1, (n_emb + EMBSPLIT - 1) / EMBSPLIT) *
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                cuda_layernorm(n_emb, buffer1, layernorms, 0, ccmean,
                               ccvariance, x, tokenlength, item_ct1);
            });
    }
    ...
    for (unsigned long long i = 0; i < n_layers; i++)
    {
...

        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    cuda_layernorm(n_emb, x, layernorms, 4 * (i) + 2, camean,
                                   cavariance, buffer1, tokenlength, item_ct1);
                });
        }
        // buffers to 0

        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    mixatt(n_emb, buffer1, statexy, mixk, mixv, mixr,
                           ffnrbuffer, i, buffer2, buffer3, buffer4, n_layers,
                           tokenlength, item_ct1, mode);
                });
        }
        ...
        // buffer1 = rwkv
        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    setx(n_emb * tokenlength, x, buffer2, item_ct1, 0);
                });
        }
        ...
        // buffer2 = attout(rwkv) + x
        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    setx(n_emb * tokenlength, buffer2, x, item_ct1, 0);
                });
        }
        ...
        std::tie(zzmean, zzvariance) = meanvar(n_emb, x, ffnrbuffer, tokenlength);
        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    cuda_layernorm(n_emb, x, layernorms, 4 * (i + 1), zzmean,
                                   zzvariance, buffer1, tokenlength, item_ct1);
                });
        }
        ...
        // ffnmixk, ffnmixv = mix(buffer1, statedd[n_emb*y], ffnmixkvr)
        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    mixffn(n_emb, buffer1, statedd, ffnmixk, ffnmixv,
                           ffnkbuffer, ffnvbuffer, i, n_layers, tokenlength,
                           item_ct1, mode);
                });
        }
        ...
        q_ct1.parallel_for(
            sycl::nd_range<3>(
                ...
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                cuda_memset(n_emb * tokenlength, buffer2, 0, item_ct1);
            });
        cudac_mm8_one(n_emb, n_emb, ffnvbuffer, ffnr, n_emb, buffer2, ffnrr, ffnro, i, tokenlength);
        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    ...
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    sigmoid(n_emb * tokenlength, buffer2, buffer4, ffnrbuffer,
                            item_ct1);
                });
        }
        ...
        q_ct1.parallel_for(
            sycl::nd_range<3>(
                sycl::range<3>(
                    1, 1, (tokenlength * n_emb * 4 + EMBSPLIT - 1) / EMBSPLIT) *
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                cuda_relusquared(tokenlength * n_emb * 4, ffnrbuffer, buffer3,
                                 item_ct1);
            });
        ...
        // buffer3 = ffnrbuffer @ ffnv
        {
            ...
            q_ct1.parallel_for(
                sycl::nd_range<3>(
                    sycl::range<3>(
                        1, 1, (tokenlength * n_emb + EMBSPLIT - 1) / EMBSPLIT) *
                        sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
                    sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
                [=](sycl::nd_item<3> item_ct1) {
                    blockout(tokenlength * n_emb, x, buffer3, buffer4,
                             item_ct1);
                });
        }
    }
    ...
    std::tie(mean, variance) = meanvar(n_emb, x, ffnrbuffer, tokenlength);
    {
        ...
        q_ct1.parallel_for(
            sycl::nd_range<3>(
                ...
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
            [=](sycl::nd_item<3> item_ct1) {
                cuda_layernorm(n_emb, x, layernorms, 4 * (n_layers) + 2, mean,
                               variance, buffer1, tokenlength, item_ct1);
            });
    }
    ...
    q_ct1.parallel_for(
        sycl::nd_range<3>(
            sycl::range<3>(1, 1,
                           (tokenlength * 50277 + EMBSPLIT - 1) / EMBSPLIT) *
                sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK),
            sycl::range<3>(1, 1, EMBSPLIT / EMBBLOCK)),
        [=](sycl::nd_item<3> item_ct1) {
            cuda_memset(tokenlength * 50277, buffer2, 0, item_ct1);
        });
...
sycl::queue &q_ct1 = dev_ct1.in_order_queue();
    ...
    for (unsigned long long i = 0; i < 46; i++)
    {
        ...
        if (!isBuffer){
            cuda_mem = (void *)sycl::malloc_device(size * Mtypes(i), q_ct1);
            q_ct1.memcpy(cuda_mem, ptrs[i], size * Mtypes(i)).wait();
        }else{
            cuda_mem = (void *)sycl::malloc_device(size * Mtypes(i) * maxGPT, q_ct1);
            for(unsigned long long j = 0; j < maxGPT; j++){
                q_ct1.memcpy(&((int *)cuda_mem)[j * size], ptrs[i], size * Mtypes(i)).wait();
            }
        }
...
void freeTensors(int **ptrs)
{
    for (unsigned long long i = 0; i < 46; i++)
    {
        ...
        dpct::dpct_free(ptrs[i], dpct::get_in_order_queue());
    }
}